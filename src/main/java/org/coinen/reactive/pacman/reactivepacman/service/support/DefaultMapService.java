...
public class DefaultMapService implements MapService {
    ...
    static List<Point> generateTileCoordinates(int width, int height, int offsetX,
        int offsetY) {
        var minX = offsetX;
        ...
        for (var i = minX ; i <= maxX ; i++) {
            for (var j = minY ; j <= maxY ; j++) {
                arr.add(Point.newBuilder().setX(i).setY(j).build());
            }
        }
    ...
    private static final List<Tile> generate(int width, int height, int offset) {
        ...
        tileIndices.sort((p1, p2) -> Float.compare(distance2(p1, center), distance2(p2, center)));
