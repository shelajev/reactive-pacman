            ...
            disposable = locationStream
                ...
                .consume(
                    (location) => {
                        if(!isFetching) {
                            isFetching = true;
                            fetch(`${endpoint || "http://URL_REPLACE_ME:3000"}/http/locate`, {
                                method: "POST",
    ...
    players(): Flux<Player.AsObject> {
        if (!this.sharedPlayersStream) {
            ...
            Flux.from<Player>(FlowableAdapter.wrap(new Flowable(subscriber => {
                const eventSource = new EventSource(`${endpoint || "http://URL_REPLACE_ME:3000"}/http/players`, { withCredentials : true });
